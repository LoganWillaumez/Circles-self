...
<script lang="ts">
  ...
  const writeMessageOnEnter = (event: CustomEvent) => {
    ...
    if (writeMessage) {
      socket.emit('socketClient', {type: 'writingMessage', data: {customer_id:user?.customer_id, name: user?.firstname, content: 'is writing...', writing: true}});
    }
    if (timeoutId) {
      clearTimeout(timeoutId);
    }
        ...
        <div class="flex flex-col gap-3 h-full">
            ...
            <div class="bg-[var(--color-chat)] w-full h-full rounded-md flex flex-col items-center px-5 py-5 overflow-hidden relative">
              <div class="absolute top-5 right-5 w-[250px] h-[50px]">
                ...
                <div class="h-[30px] w-[30px] absolute top-1/2  right-0" 
                style="transform: translate({index * -20}px, -50%);">
                    <img class="h-[30px] w-[30px] rounded-full" alt='profile' src={actualCircle.data.customerDatas.find(userCircle => userCircle.customer_id === user.customer_id)?.img}>
                ...
                <div transition:fade={{ duration: 300 }}>
                  ...
                  class:message-join={!message.customer_id && !message.writing}>
                    {#if message.customer_id && allUserConnected.length > 0}
                      <img class="h-[30px] w-[30px] rounded-full" alt='profile' src={actualCircle.data.customerDatas.find(user => user.customer_id === message.customer_id)?.img}>
                    {/if}
