  ...
  describe('Customer Data Manipulation', () => {
    ...
    describe('Create User', () => {
      ...
      test('should fail when required fields are missing', async () => {
        ...
        await expect(customerDataMapper.createUser(partialUserData as unknown as CustomerInputDatas)).rejects.toThrowError();
      });
      test('should fail when email already exists', async () => {
        await expect(customerDataMapper.createUser(createdUser)).rejects.toThrowError();
      });
      test('should fail when birthdate format is invalid', async () => {
        const invalidBirthdateUser = { ...createdUser, birthdate: '2022-13-32' };
        await expect(customerDataMapper.createUser(invalidBirthdateUser)).rejects.toThrowError();
      });
      test('should fail when gender value is invalid', async () => {
        const invalidGenderUser = { ...createdUser, gender: 'INVALID' };
        await expect(customerDataMapper.createUser(invalidGenderUser)).rejects.toThrowError();
      });
      test('should fail when a required field has an empty string value', async () => {
        const emptyFieldUser = { ...createdUser, firstname: '' };
        await expect(customerDataMapper.createUser(emptyFieldUser)).rejects.toThrowError();
      });
    ...
    describe('Get Customer', () => {
      describe('Get customer by ID', () => {
        test('should return customer data when id is valid', async () => {
          const customer = await customerDataMapper.getCustomerById(createdUser.customer_id);
      ...
      describe('getCustomerByEmail', () => {
        test('should return customer data when email is valid', async () => {
          const customer = await customerDataMapper.getCustomerByEmail(createdUser.email!);
          expect(customer).toMatchObject({
            customer_id: createdUser.customer_id,
            firstname: createdUser.firstname,
            lastname: createdUser.lastname,
        ...
        test('should not found customer when email is invalid', async () => {
          ...
          const customer = await customerDataMapper.getCustomerByEmail(invalidEmail);
...
describe('Patch User', () => {
  test('should update a user field and return the updated user data', async () => {
    const userToUpgrade = await customerDataMapper.createUser(generateCustomer());
    if(!userToUpgrade){
      fail("No User to Update");
    }
    const updatedFirstname = 'UpdatedFirstname';
    const updatedUser = await customerDataMapper.patchUser(userToUpgrade.customer_id, { firstname: updatedFirstname });
  ...
  test('should update multiple user fields and return the updated user data', async () => {
    const userToUpdate = await customerDataMapper.createUser(generateCustomer());
    ...
    const updatedUser = await customerDataMapper.patchUser(userToUpdate.customer_id, updatedData);
  ...
  test('should not update user when id is invalid', async () => {
    const invalidId = -1;
    await expect(customerDataMapper.patchUser(invalidId, { firstname: 'UpdatedFirstname' })).resolves.toBe(false);
  });

  test('should not update user when no fields are provided', async () => {
    const emptyData = {};
    const updatedUser = await customerDataMapper.patchUser(createdUser.customer_id, emptyData);
  ...
  test('should throw an error when trying to update a non-existing field', async () => {
    ...
    await expect(customerDataMapper.patchUser(createdUser.customer_id, nonExistingFieldData)).rejects.toThrowError();
...
describe('Delete Customer', () => {
  test('should delete customer with the given id', async () => {
    ...
    const createdUserToDelete = await customerDataMapper.createUser(generateCustomer());


    // Delete the created customer
    if(createdUserToDelete) {
      await customerDataMapper.deleteCustomer(createdUserToDelete.customer_id);
      
  
      // Try to find the deleted customer
      const deletedCustomer = await customerDataMapper.getCustomerById(createdUserToDelete.customer_id);
  ...
  test('should not throw an error when the customer id does not exist', async () => {
    ...
    await expect(customerDataMapper.deleteCustomer(nonExistentCustomerId)).resolves.not.toThrow();
...
describe('validUser', () => {
  ...
  test('should return false if already activated', async () => {
    ...
    const createToActivate = await customerDataMapper.createUser(generateCustomer());

    if(createToActivate) {
      await customerDataMapper.validUser(createToActivate.identifier); // first activation
      const checkActivatedUser = await customerDataMapper.validUser(createToActivate.identifier);
  ...
  test('should return false when identifier does not exist', async () => {
    ...
    const validUser = await customerDataMapper.validUser(nonExistentIdentifier);
  ...
  test('should activate the user when the email validation is within 24 hours', async () => {
    ...
    const createdRecentUser = await customerDataMapper.createUser(generateCustomer());

    // Call validUser function and expect it to return true
    if(createdRecentUser) {
      const validUser = await customerDataMapper.validUser(createdRecentUser.identifier);
      expect(validUser).not.toBe(false);
      const updatedUser = await customerDataMapper.getCustomerById(createdRecentUser.customer_id);
      updatedUser && expect(updatedUser.activated_at).not.toBe(null);
  
      // Delete the created user
      await customerDataMapper.deleteCustomer(createdRecentUser.customer_id);
    }
 ...
 test('should not activate the user when the email validation is older than 24 hours', async () => {
  ...
  const createdOutdatedUser = await customerDataMapper.createUser(generateCustomer());
  ...
  if(createdOutdatedUser) {
    await customerDataMapper.patchUser(createdOutdatedUser.customer_id, { email_valid: outdatedEmailValidDate });
  
    // Call validUser function and expect it to return false
    const validUser = await customerDataMapper.validUser(createdOutdatedUser.identifier);
    expect(validUser).toBe(false);
  
    // Check that the user has not been activated
    const nonUpdatedUser = await customerDataMapper.getCustomerById(createdOutdatedUser.customer_id);
    nonUpdatedUser ? expect(nonUpdatedUser.activated_at).toBe(null) : fail("No Update User");
  
    // Delete the created user
    await customerDataMapper.deleteCustomer(createdOutdatedUser.customer_id);
  } else {
...
describe('reloadEmailValid', () => {
  test('should update the email_valid field and return the new value', async () => {
    const createdUser = await customerDataMapper.createUser(generateCustomer());

    if(createdUser) {
      const updatedEmailValid = await customerDataMapper.reloadEmailValid(createdUser.customer_id);
      console.log('🚀 ~ updatedEmailValid:', updatedEmailValid);
      const updatedUser = await customerDataMapper.getCustomerById(createdUser.customer_id);
      console.log('🚀 ~ updatedUser:', updatedUser);
...
describe('checkActivatedAtByEmail', () => {
  test('should return the activated_at field when email is valid', async () => {
    ...
    const activatedAt = await customerDataMapper.checkActivatedAtByEmail(createdUser.email!);
    const updatedUser = await customerDataMapper.getCustomerById(createdUser.customer_id);
  ...
  test('should return false when email is invalid', async () => {
    const invalidEmail = 'invalid@example.com';
    const activatedAt = await customerDataMapper.checkActivatedAtByEmail(invalidEmail);
...
describe('checkActivatedAtByIdentifier', () => {
  test('should return the activated_at field when identifier is valid', async () => {
    ...
    const activatedAt = await customerDataMapper.checkActivatedAtByIdentifier(createdUser.identifier);
    const updatedUser = await customerDataMapper.getCustomerById(createdUser.customer_id);
    if(!updatedUser) {
      fail("No updatedUser User");
    }
  ...
  test('should return false when identifier is invalid', async () => {
    ...
    const activatedAt = await customerDataMapper.checkActivatedAtByIdentifier(invalidIdentifier);
