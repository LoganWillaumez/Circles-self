import { CirclesDatas } from '../../../ts/interfaces/circle.interfaces';
...
describe('Circles Data Mapper - Integration Tests', () => {
  ...
  beforeAll(async () => {
    ...
    const circleGet = await circlesDataMapper.createCircle(createdUser.customer_id, testCircle);
   if(!circleGet) {
  ...
  describe('Create a circle', () => {
    test('should create a circle and return his datas', async () => {
      const createdCircle = await circlesDataMapper.createCircle(createdUser.customer_id, circleTest);
  ...
  test('getCircle', async () => {
    const retrievedCircle = await circlesDataMapper.getCircle(circleTest.circle_id);
  ...
  test('updateCircle', async () => {
    ...
    const updatedCircle = await circlesDataMapper.updateCircle(circleTest.circle_id, updatedData);
  ...
  test('deleteCircle', async () => {
    ...
    const newAdminCustomer = await customerDataMapperInstance.test.createUser(generateCustomer());
    if(!newAdminCustomer){
      fail('Customer creation for deletion failed')
    }
    const circleToDelete = await circlesDataMapper.createCircle(newAdminCustomer.customer_id, testCircle);
    if (circleToDelete !== false) {
      await circlesDataMapper.deleteCircle(circleToDelete.circle_id);
      const retrievedCircle = await circlesDataMapper.getCircle(circleToDelete.circle_id);
